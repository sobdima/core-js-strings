...
function isPalindrome(str) {
  ...
  return lowerCaseStr
    .split('')
    .every(
      (char, index) => char === lowerCaseStr[lowerCaseStr.length - index - 1]
    );
}
...
function findLongestWord(sentence) {
  const resultArray = sentence.split(/\s+/).sort((a, b) => {
...
function reverseWords(str) {
  ...
  for (let i = 0; i < wordsArray.length; i += 1) {
    newArr.push(wordsArray[i].split('').reverse().join(''));
  }
  return newArr.join(' ');
}
...
function extractNameFromTemplate(value) {
  const strLength = value.length - 1;
  return value.substring(7, strLength);
}
